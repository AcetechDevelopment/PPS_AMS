...
const SpareAssign = () => {
  ...
  const submitvichile = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}tyreassign/create`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setShow(false);
        setsave_data({ type: "", vehicle_id: "", brand: "", modal: "", partnumber: "", serialno: "", location: "" });
      } else {
  ...
  const fetchData = async ({ fetchpageSize=15, fetchpageIndex}) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}tyreassign/list?start=${start}&limit=${limit}&search=${search}&order_by=${orderBy}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: 'Type', accessor: info => info.type === 1 ? 'Vehicle' : 'Trailer' },
  ...
  const viewvehicle = async (id) => {
    try {
      const response = await fetch(`${BASE}tyreassign/history/${id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${authToken}`,
  ...
  const deletevehicle = async (id) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await fetch(`${BASE}tyreassign/delete/${id}`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${authToken}`,
  ...
  const fetchvehicle_search = async (search) => {
    ...
    const type = save_data.type;
    try {
      const url = `${BASE}options/search_vehicle/${encodeURIComponent(search)}/${type}`;
  ...
  const fetchbrand_search = async (search) => {
    ...
    try {
      const url = `${BASE}options/brand_tyre/${encodeURIComponent(search)}`;
      const response = await fetch(url, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
