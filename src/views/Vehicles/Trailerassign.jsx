...
const Trailerassign = () => {
  ...
  const submitvichile = async () => {
    ...
    try {
      const response = await fetch(`${BASE}trailerassign/create`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(result)
        toast.success('Trailer assign done!');
...
const fetchtrailer_search = async (search) => {
  ...
  const fetchData = async ({ pageSize = 15, pageIndex = 0, sortBy = [], search = '', todate, location } = {}) => {
    ...
    try {
      const url = `${BASE}trailerassign/list?start=${start}&limit=${limit}&search=${encodeURIComponent(search)}&order_by=${encodeURIComponent(orderBy)}`;
      const response = await fetch(url, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
  ...
  const columns = useMemo(
    () => [
      ...
      { Header: 'Vehicle Number', accessor: 'vehicle_id' },
      
      { Header: 'Trailer Number', accessor: 'trailer_id' },
      {
        Header: () => <FaBars />,
        id: 'actions',
        Cell: ({ row }) => {
  ...
  const updatevichile = async () => {
    ...
    const verify = vehicleNum(data.vehicle_id);
    if (!verify.isValid) {
  ...
  const handlePrint = async () => {
    try {
      ...
      const response = await fetch(`${BASE}trailerassign/list?start=0&limit=1000`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${authToken}`,
        },
  ...
  setselectedvalues((prev) => ({
    ...
    trailer_id: "" // reset when vehicle changes
  }));
}}
                  />
