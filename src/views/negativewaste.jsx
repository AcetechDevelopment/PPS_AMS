...
const Negativewaste = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${base_url}neg_waste/create`, {
        method: 'POST',
        headers: {
  ...
  const fetchData = async ({ pageSize, pageIndex, sortBy, fromdate, todate, location }) => {
    ...
    try {
      const response = await fetch(
        `${base_url}neg_waste/list?pageSize=${pageSize}&pageIndex=${pageIndex+1}&sortBy=${sortColumn}&sortOrder=${sortOrder}&from=${fromdate}&to=${todate}&location=${location}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
